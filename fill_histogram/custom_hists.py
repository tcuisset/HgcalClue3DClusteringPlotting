...
class RechitsPositionXY(MyHistogram):
    def __init__(self) -> None:
        super().__init__(beamEnergiesAxis, layerAxis, 
            hist.axis.Regular(bins=50, start=-10., stop=10., name="rechits_x", label="RecHit x position"), 
            hist.axis.Regular(bins=50, start=-10., stop=10., name="rechits_y", label="RecHit y position")
        )
        self.label = "RecHits position"
        self.profileOn = ProfileVariable('rechits_energy', 'Reconstructed hit energy')
...
class Clus3DPositionXY(MyHistogram):
    def __init__(self) -> None:
        ...
        self.label = "3D cluster X-Y position"
        self.profileOn = ProfileVariable('rechits_energy', '3D cluster total reconstructed energy')
...
class Clus3DSpatialResolution(MyHistogram):
    def __init__(self) -> None:
        ...
        self.label = "Spatial resolution of 2D clusters that are part of a 3D cluster"
        self.profileOn = ProfileVariable('clus2D_energy', '2D cluster energy')
